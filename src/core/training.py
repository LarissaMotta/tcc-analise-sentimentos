import src.core.process_data as process_file
...
def training(df, df2, df3):
    ...
    dic_freq, df, df2, df3 = process_file.process_data_freq(df, df2, df3)
    ...
    net = LSTM_model.SentimentLSTM(n_vocab, n_embed, n_hidden, n_output, n_layers)
    ...
    print("Ultimo Loss: {:.6f}".format(list_loss_validation[-1]),
          ...
          "Loss Médio: {:.6f}".format(sum(list_loss_validation) / len(list_loss_validation)))
    print("Ultima Acurácia: {:.6f}".format(list_acc_validation[-1]),
          "Acurácia Máxima: {:.6f}".format(max(list_acc_validation, key=float)),
          "Acurácia Média: {:.6f}".format(sum(list_acc_validation) / len(list_acc_validation)))

    ## TODO PLOT GRAPHS

    return net, df3, batch_size, criterion
